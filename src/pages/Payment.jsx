...
export default function Payment(){
  ...
  return <Storefront><main className="payment-page"><div className="page-title"><span className="eyebrow">PAYMENT</span><h1>Pay for your order</h1><p>Order #{order.id} · Total {money(order.total)}</p></div>
    ...
    </section><aside className="security-panel"><div className="secure-icon">✓</div><h3>How MVEC works</h3><p>For this protected-settlement flow, your payment is recorded as HELD by MVEC until delivery is confirmed. In production, the protected funds must be held by an appropriately regulated payment partner.</p><div className="status-flow"><span>Payment</span><i>→</i><span>MVEC holds</span><i>→</i><span>Delivery</span><i>→</i><span>Release</span></div><p className="tiny">Delivery proof, reviews and dispute support help keep the marketplace trustworthy.</p></aside></div>
  </main></Storefront>
}